...
const contentsUrl = (slug) =>
  `https://api.github.com/repos/${REPOSITORY}/contents/src/content/posts/${encodeURIComponent(slug)}.md`;
...
export async function onRequestDelete({ request, env }) {
  ...
  if (!validSlug(slug)) return json({ message: "Invalid blog post address." }, 400);
  ...
  try {
    ...
    if (existingResponse.ok) {
      ...
      if (!deleteResponse.ok) {
        let message = `Blog post could not be deleted (${deleteResponse.status}).`;
        try {
      ...
      return json({ message: `Blog post could not be checked (${existingResponse.status}).` }, existingResponse.status);
    }

    if (hasContentStore(env)) {
      await ensureContentSchema(env);
      const result = await env.CONTENT_DB.prepare(
        "DELETE FROM content_entries WHERE type = 'post' AND slug = ?1"
      ).bind(slug).run();
      d1Deleted = Number(result?.meta?.changes || 0) > 0;
    }

    if (!githubDeleted && !d1Deleted) {
      return json({ message: "Blog post was not found." }, 404);
    }

    return json({ deleted: true, slug, githubDeleted, d1Deleted });
  } catch (error) {
    return json({ message: error?.message || "Blog post could not be deleted." }, 500);
  }
}
